SELECT 
    fsmo.id AS fsm_order,  -- ID de la orden FSM
    fsmo.name AS name_fsmo,  -- Nombre de la orden FSM
    location.id AS kiosk_id,  -- ID de la ubicación del kiosko
    location.complete_name AS name_kiosk,  -- Nombre completo de la ubicación del kiosko
    location_partner.ref AS reference,  -- Referencia del partner asociado a la ubicación del kiosko
    assigned_partner.name AS assigned_to,  -- Nombre del partner asignado a la orden
    ft.name AS team,  -- Nombre del equipo asignado a la orden
    location_partner.name AS partner_name,  -- Nombre del partner asociado a la ubicación del kiosko
    stock_location.name AS inventory_location_name,  -- Nombre de la ubicación de inventario asociada a la orden
    sw.name AS warehouse_name,  -- Nombre del almacén asociado a la orden
    sw.code AS warehouse_code,  -- Código del almacén asociado a la orden
    fs_stage.name AS stage_name,  -- Nombre del estado o etapa de la orden
...
GROUP BY 
    fsmo.id,  -- Agrupa por ID de la orden FSM
    fsmo.name,  -- Agrupa por nombre de la orden FSM
    location.id,  -- Agrupa por ID de la ubicación del kiosko
    location_partner.ref,  -- Agrupa por referencia del partner asociado a la ubicación del kiosko
    ft.name,  -- Agrupa por nombre del equipo
    assigned_partner.name,  -- Agrupa por nombre del partner asignado a la orden
    location_partner.name,  -- Agrupa por nombre del partner asociado a la ubicación del kiosko
    stock_location.name,  -- Agrupa por nombre de la ubicación de inventario
    sw.name,  -- Agrupa por nombre del almacén
    sw.code,  -- Agrupa por código del almacén
    fs_stage.name  -- Agrupa por nombre de la etapa
ORDER BY 
    fsmo.id;  -- Ordena los resultados por el ID de la orden FSM